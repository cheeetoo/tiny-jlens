...
REF_DATA = ROOT / "ref/jacobian-lens/data/experiments"  # the paper's released prompt data
RESULTS = ROOT / "results"                              # one subdirectory per criterion
# the authors' released gpt2-small lens (not in git; see lenses/gpt2-small/config.yaml)
LENS_PATH = pathlib.Path(os.environ.get(
    "JLENS_LENS",
    ROOT / "lenses/_hf/gpt2-small/jlens/Salesforce-wikitext/gpt2_jacobian_lens.pt"))
...
class Lensed:
    """The model, its lens, and the (centered) J-lens dictionary.  Everything fp32 on `device`."""

    def __init__(self, device: str | None = None):
        # DEVICE=... overrides; otherwise the GPU when there is one.
        self.device = device or os.environ.get("DEVICE") or (
