...
Writes results/c2_modulation/{results.json, prompts.json, summary.txt}.
Run:  python -m jl.c2_modulation
"""
...
def concept_trial(lm, carrier, instruction, x, tids):
    ...
    ids, span = copy_frame(lm, carrier, instruction, x)
...
def main():
    ...
    words = concept_words(lm)[:N_WORDS]
    carriers = CARRIERS[:N_CARRIERS]

    # ============================================================ 2a  concept modulation
    for w in words:
        tids = word_targets(lm, w)
        for ci, carrier in enumerate(carriers):
            for cond in CONDITIONS:
                phrasings = [None] if cond == "baseline" else PHRASINGS[cond]
    ...
    for c in TOPICS:
        mt = sorted({t for m in c["members"] if lm.is_single(" " + m)
                     for t in word_targets(lm, m)})
        if not mt:
            continue
        for ci, carrier in enumerate(carriers):
            for cond, instr in (("baseline", None), ("focus", PHRASINGS["focus"][0]),
                                ("mention", PHRASINGS["mention"][0])):
    ...
    for it in TD["items"]:
        ...
        q1_ids = torch.tensor([[lm.bos] + lm.tok(TD["q1"] + "\n" + it["stimulus"],
        ...
        for tag, question in (("q1", TD["q1"]), ("q2", it["q2"])):
    ...
    for se, sf in zip(IMAGINE["en"], IMAGINE["fr"]):
        ...
        rec["neutral"] = [imagine_measures(h.format(s=se), se) for h in IMAGINE_HEADERS["neutral"]]
        rec["claim"] = [imagine_measures(h.format(s=se), se) for h in IMAGINE_HEADERS["claim"]]
        rec["real"] = [imagine_measures(h.format(s=sf), sf) for h in IMAGINE_HEADERS["neutral"]]
        out["imagine"].append(rec)

    # ------------------------------------------------------------ write
    R = jl.results_dir("c2_modulation")
    json.dump(out, open(R / "results.json", "w"))
    # a few example prompts, verbatim, for inspection
    ex = []
    w0, c0 = words[0], carriers[0]
    for cond in CONDITIONS:
        instr = None if cond == "baseline" else PHRASINGS[cond][0]
        ids, span = copy_frame(lm, c0, instr, w0)
    ...
    ex.append(dict(exp="2d", cond="claim",
                   prompt=IMAGINE_HEADERS["claim"][0].format(s=IMAGINE["en"][0])))
    json.dump(ex, open(R / "prompts.json", "w"), indent=1)
...
def summarize(out, words, carriers):
    ...
    for cond in CONDITIONS:
        ...
        L.append(f"    {cond:10s} {KIND[cond]:9s}  {h(1):.2f}   {h(5):.2f}  {h(10):.2f}  {h(25):.2f}"
    ...
    L.append("       (so the property IS used automatically -- the model just isn't asked to name it)")
    L.append(f"    {'item':10s}  q2 hit@10  q1 hit@10   (stimulus positions with an expected label in band lens top-10)")
    for it in TD["items"]:
        d2 = next(d for d in DMD if d["key"] == it["key"] and d["q"] == "q2")
        d1 = next(d for d in DMD if d["key"] == it["key"] and d["q"] == "q1")
