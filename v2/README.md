...
c5_selectivity/      criterion 5: selectivity  (same layout)
  PROTOCOL.md        paper text <-> what we ran <-> deviations, with results
  prompts.py         battery tasks (two-hop copied from c3, one-hop, induction, wikitext) + language passages
  ablation.py        the top-k J-space ablation + matched-norm random control (§3.5.2), local to c5
  run.py             the whole criterion in one run -> results/{results.json, prompts.json, summary.txt}
  figure.py          -> figures/criterion5.png
```
...
`python c5_selectivity/run.py && python c5_selectivity/figure.py` (a few minutes on one GPU).
