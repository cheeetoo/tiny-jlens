"""Figures for criterion 2.  Run after run.py.  Produces:
  figures/criterion2.png        full 4-panel (A ladder, B per-layer, C privileging, D floors)
  figures/criterion2_slide.png  2-panel slide version (A + C only)

Panels:
A  the condition ladder (2a): fraction of trials the word reaches the workspace band (top-25),
   by condition; dots = per-phrasing rates.  mention primes, focus adds, "ignore" does nothing,
   "don't think" backfires.
B  per-layer localization (2a): median best rank across L6..L10; band 7-9 shaded.  The effect
   strengthens toward the motor layer L10 (the caveat, made visible).
...
def panel_A(ax):
    ...
    for i, cond in enumerate(CONDS):
        ...
        if cond != "baseline":
            for p in sorted({r["phrasing"] for r in C if r["cond"] == cond}):
...
# ------------------------------------------------------------------ 2-panel slide (A + C)
sfig, saxes = plt.subplots(1, 2, figsize=(13, 5.2))
panel_A(saxes[0])
panel_C(saxes[1])
sfig.suptitle("Criterion 2 — directed modulation — gpt2-small", y=1.02, fontsize=13)
sfig.tight_layout()
sfig.savefig(f"{HERE}/figures/criterion2_slide.png", dpi=160, bbox_inches="tight")
print("wrote figures/criterion2.png and figures/criterion2_slide.png")
